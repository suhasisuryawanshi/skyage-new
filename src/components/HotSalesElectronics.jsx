...
export default function HotSalesElectronics(){
  ...
  useEffect(()=>{
    ...
    getAllProducts()
      .then(data => { 
        if(mounted){
          const electronics = (data || []).filter(p => (p.category || '').toLowerCase() === 'electronics')
  ...
  return (
    <section className="container mx-auto px-4 py-6  bg-white" style={{ borderTop: '3px solid #147E9E' }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold">Hot Sales on Electronics</h2>
        <Link to="/products?category=electronics" className="text-sm text-[#147E9E]">{t('viewAll')}</Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-stretch h-[560px]">
        <div className="md:col-span-2 row-span-2 flex items-center justify-center">
          <Link to="/products?category=electronics" aria-label="See all electronics" className="block">
            <div className="w-[325px] h-[432px] rounded overflow-hidden relative shadow-lg">
              <img src={promoImage} alt="Hot Sales Electronics" className="w-full h-full object-cover" />
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="bg-white/90 text-center px-4 py-2 rounded">
                  <span className="text-sm font-semibold">See All Electronics</span>
                </div>
              </div>
            </div>
          </Link>
